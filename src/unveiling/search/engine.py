...
from unveiling.search.serper import search as serper_search
...
def search(query: str, num: int = 8, far_search_hint: str = "", lang: str = "") -> list[dict]:
    ...
    if engine == "wikipedia":
        from unveiling.search.wikipedia import search as wikipedia_search
        return wikipedia_search(query, num=num, far_search_hint=far_search_hint, lang=lang)

    if engine == "exa":
        from unveiling.search.exa import search as exa_search
        try:
    ...
    from unveiling.search.llm_fallback import search as llm_fallback_search
